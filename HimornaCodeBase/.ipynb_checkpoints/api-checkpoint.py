...
def _search_list(tx, query:str)->List[List[Union[str,int]]]:
    result = tx.run(query)
    return [[ record["hm"][2],  record["lnc"],  record["name"],  record["chrom"], str(record["start"]), str(record["end"]), str(record["gene"]), str(record["corr"])] for record in result]
...
def lncrna_raw():
    ...
    writer.writerow(["Histone Modification", "lncRNA", "Peak Id", "Chr", "Start", "End", "Gene", "Corr"])
    ...
    response.headers["Content-Disposition"] = f"attachment; filename=peaks_{lncrna}.csv"
...
def corr():
    ...
    res_signal.update( {r["tissue"] : r["signal"] for r in res})
    res_expression = dict()
    res_expression[""] = "Gene expression"
    res_expression.update( {r["tissue"] : r["expression"] for r in res})
